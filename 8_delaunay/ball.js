class Ball {
	...
	draw() {
		for (let i = 1; i < this.prevPos.length; i++) {
			stroke(0, 150, 0, map(i, 0, this.prevPos.length, 40, 255));
			// strokeWeight(map(i, 0, this.prevPos.length, 1, 5));
	...
	accel() {
		...
		let dmax = 1000000000000;
		let closePt = 0;
		// console.log('objs num ', objs.length);

		for (let i = 0; i < objs.length; i++) {
			let pt = this.mappaObj.latLngToPixel(objs[i]);
			let between = new p5.Vector(pt.x, pt.y);
			between.sub(this.pos);
			let d = between.magSq(this.pos.x, this.pos.y);
			if (d < dmax) {
				dmax = d;
				closePt = i;
			}
			// console.log('dist=', d, ' ', dmax, ' ', closePt, ' ', i);
		}
		if (dmax < 1000000000000) {
			// console.log(closePt);
			let pt = this.mappaObj.latLngToPixel(objs[closePt]);
			target.x = pt.x;
			target.y = pt.y;
		}
		target.sub(this.pos);
	...
	update() {
		...
		if (this.prevPos.length > settingsObject.trailLength) {
			this.prevPos.shift();
		}
		if(this.prevPos.length > settingsObject.trailLength) {
			this.prevPos.splice(0, this.prevPos.length - settingsObject.trailLength);
		}
